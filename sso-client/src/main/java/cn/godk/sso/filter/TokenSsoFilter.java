...
public class TokenSsoFilter extends AbstractSsoFilter {
    ...
    /**
     * 当前系统未登录，但是浏览器可以获取到登陆状态且登陆成功，成功后操作 (用于子类重写 设置 session 用户信息)
     *
     * @param request  req
     * @param response res
    ...
    public void operation(ServletRequest request, ServletResponse response, Permit permit) {
        HttpServletRequest req = (HttpServletRequest) request;
        req.getSession().setAttribute("username", permit.getUsername());
        req.getSession().setAttribute("token", permit.getKey());
    }
